//
//  ShowTokensListSegue.swift
//  BankexWallet
//
//  Created by Георгий Фесенко on 27/06/2018.
//  Copyright © 2018 Alexander Vlasov. All rights reserved.
//

import UIKit

class ShowTokensSegue: UIStoryboardSegue {
    override func perform() {
        transition()
    }
    
    func transition() {
        let toViewController = self.destination as! TokensListController
        let fromViewController = self.source
        
        let screenWidth: CGFloat = UIScreen.main.bounds.size.width - 28 * 2
        let screenHeight: CGFloat = UIScreen.main.bounds.size.height / 3
